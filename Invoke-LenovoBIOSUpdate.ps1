...
Process {
	...
	if (($TSEnvironment -ne $null) -and ($TSEnvironment.Value("_SMSTSinWinPE") -eq $true)) {
		try {
			# https://docs.lenovocdrt.com/ref/bios/bios_guide/#os-optimized-defaults
			# drvload %systemroot%\INF\Battery.inf
			try {
				Start-Process -FilePath drvload -ArgumentList "$($env:SystemRoot)\INF\Battery.inf" -Passthru -Wait
			} catch {
				Write-CMLogEntry -Value "An error occured while loading the battery info. Error message: $($_.Exception.Message)" -Severity 3
			}
			
			# Start flash update process
			Write-CMLogEntry -Value "Calling: $FlashUtility with argument: $($FlashSwitches)" -Severity 1
			...
			$FlashProcess.ExitCode | Out-File -FilePath $LogFilePath -ErrorAction SilentlyContinue
			...
			if ($WinUPTPLog -ne $null) {
				...
				$SMSTSLogPath = Join-Path -Path $TSEnvironment.Value("_SMSTSLogPath") -ChildPath "winuptp.log" ErrorAction SilentlyContinue
