...
$whoami = $env:USERNAME
$IntuneFolder = "$env:ProgramData\Microsoft\IntuneManagementExtension\"
$LogFilePath = "$IntuneFolder\Logs"
$LogFile = "$LogFilePath\$ScriptName.log"
$Remediate = $true
if ($Remediate -eq $true){ $ComponentText = "Intune - Remediation" } else {$ComponentText = "Intune - Detection"}

if (!(Test-Path -Path $LogFilePath)){ New-Item -Path $LogFilePath -ItemType Directory -Force }
...
try {
	CMTraceLog -Message "Attempting to install latest NuGet package provider" -Type 1 -LogFile $LogFile
	$PackageProvider = Install-PackageProvider -Name "NuGet" -Force -ErrorAction Stop -Verbose:$false
}
catch [System.Exception] {
	CMTraceLog -Message "Unable to install latest NuGet package provider. Error message: $($_.Exception.Message)" -Type 3 -LogFile $LogFile
	exit 1
} 
...
if ($HPCMSL){[Version]$InstalledHPCMSLModuleVer = $HPCMSL.Version} else { <#Write-Output "No HPCMSL PS Module" #> }
