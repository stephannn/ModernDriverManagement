...
Process {
	...
	function Get-BIOSPackages {
		...
		try {
			...
			if ($null -ne $Packages) {
				...
				return $Packages
			} elseif ($null -eq $Packages -and $OperationalMode -eq "Pilot") {
	...
	function Get-ComputerData {
		...
		switch -Wildcard ($ComputerManufacturer) {
			...
			"*HP*" {
				...
				try {
					...
					Write-CMLogEntry -Value "[BIOSPackage]: Get-ComputerData - System Exception Error: $_" -Severity 3
					$ComputerDetails.SystemSKU = (Get-ItemProperty -Path Registry::HKEY_LOCAL_MACHINE\Hardware\Description\System\BIOS -Name BaseBoardProduct).BaseBoardProduct.Trim()
				}
		...
		if ($Script:PSCmdlet.ParameterSetName -like "Debug") {
			...
			if (-not $ComputerDetails.Manufacturer -or -not $ComputerDetails.Model -or -not $ComputerDetails.SystemSKU) {
				throw "When using -DebugMode you must specify -Manufacturer, -ComputerModel and -SystemSKU."
			}
		}
	...
	function Compare-BIOSVersion {
		...
		if ($ComputerManufacturer -match "Lenovo") {
			...
			$CurrentBIOSReleaseDate = "$BIOS_Maj_Version.$BIOS_Min_Version"
					...
					if ($PackageList.Count -ge 1) {
						...
						if ($PackageList.Count -eq 1) {
							...
							if ($Script:PSCmdlet.ParameterSetName -notlike "Debug") {
								...
								Write-CMLogEntry -Value "Task sequence engine would have been instructed (if new Version is available) to download package ID $($PackageList[0].PackageID) to %_SMSTSMDataPath%\BIOSPackage" -Severity 1
							}
	...
	try {
		...
		# Fallback to 'production' if no pilot package was found
		...
		Write-CMLogEntry -Value "[BIOSPackage]: System Exception Error: $_" -Severity 3
		...
		Write-CMLogEntry -Value "[BIOSPackage]: $msg" -Severity 3
